...
const APP_VERSION = 'v4-login-fallback-2026-08-20';
...
const server = http.createServer((req,res)=>{
  ...
  if(url.pathname==='/health') return send(res,200,JSON.stringify({ok:true,version:APP_VERSION}),'application/json');
  ...
  fs.readFile(full,(e,d)=>{
    // Fallback to dashboard for unknown paths so Render/browser refresh never shows plain Not found.
    if(e) {
      full = path.join(ROOT, 'index.html');
      return fs.readFile(full, (e2,d2)=> e2 ? send(res,404,'Dashboard file missing') : send(res,200,d2,'text/html'));
    }
